...
export default function Project({
  ...
  />
  <div className="absolute inset-0 bg-gray-200 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center text-center p-2 sm:p-4">
    ...
    <div className="flex gap-2 flex-wrap">
      ...
      <Link href={githubCode} target="_blank" className="text-blue-500 hover:underline">
        Github
      </Link>
    </div>
    <ul className="flex flex-wrap gap-2 mt-4">
      {tags.map((tag, index) => (
        <li className="bg-black text-white px-2 py-1 rounded text-xs sm:text-base" key={index}>
          {tag}
        </li>
      ))}
    </ul>
